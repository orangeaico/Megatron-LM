...
export QUACK_COMPILE_WORKERS=${QUACK_COMPILE_WORKERS:-1}
export QUACK_CACHE_AUTOTUNING=${QUACK_CACHE_AUTOTUNING:-1}
export SONIC_MOE_USE_TORCH_GROUPED=${SONIC_MOE_USE_TORCH_GROUPED:-loop}
...
TOKENIZER_ARG="Qwen/Qwen3-Coder-30B-A3B-Instruct" # Path to tokenizer model, or "MOCK"
...
if [[ "$TRAINING_MODE" == "cpt" ]]; then
    ...
    TRAIN_DATA_PATH="$BASE_DIR/data/sft/saurav_hints_cleanup/training_data_30b_480b_380.jsonl"
...
TRAINING_ARGS=(
    ...
    --train-samples 2304
    --lr-decay-samples 2304

    # Learning rate args
    --lr-warmup-samples 120
    --lr 1.0e-5 # 5.0e-5
