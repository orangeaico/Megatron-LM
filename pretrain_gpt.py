...
def get_batch(data_iterator, vp_stage: Optional[int] = None):
    ...
    teacher_data = None
    if args.distillation_loss and teacher_packed is not None:
        if has_teacher_data(teacher_packed):
            teacher_data = unpack_teacher_batch(teacher_packed)

    return (*batch.values(), packed_seq_params, teacher_data)
    ...
    with stimer, mem_phase("FORWARD", do_barrier=True), nvtx_range("FORWARD"):
        if args.use_legacy_models:
            ...
            if return_schedule_plan:
                assert args.overlap_moe_expert_parallel_comm,                     "overlap_moe_expert_parallel_comm must be enabled to return the schedule plan"
...
def train_valid_test_datasets_provider(train_val_test_num_samples, vp_stage=None):
    ...
    print_rank_0(f"> train dataset: {train_ds} samples")
    print_rank_0(f"> validation dataset: {valid_ds} samples")
    print_rank_0(f"> test dataset: {test_ds} samples")
    print_rank_0("> finished creating GPT datasets ...")
    print_rank_0(
        f"> Length of train_ds: {len(train_ds) if train_ds else 0}, "
        f"valid_ds: {len(valid_ds) if valid_ds else 0}, "
        f"test_ds: {len(test_ds) if test_ds else 0}"
    )
